...
class Election(HeliosModel):
    ...
    ELECTION_TYPES = (("election", "Election"), ("referendum", "Referendum"))
    ...
    def get_or_create(cls, **kwargs):
        return cls.objects.get_or_create(
            short_name=kwargs["short_name"], defaults=kwargs
        )
    ...
    def eligibility_constraint_for(self, user_type):
        ...
        relevant_constraints = [
            ...
            if constraint["auth_system"] == user_type and "constraint" in constraint
        ]
            ...
            for constraint in self.eligibility:
                if "constraint" in constraint:
                    for one_constraint in constraint["constraint"]:
                        return_val += "<li>%s</li>" % AUTH_SYSTEMS[
                            constraint["auth_system"]
    ...
    def issues_before_freeze(self):
        ...
        if self.questions == None or len(self.questions) == 0:
            issues.append(
                {"type": "questions", "action": "add questions to the ballot"}
            )

        trustees = Trustee.get_by_election(self)
        if len(trustees) == 0:
            issues.append({"type": "trustees", "action": "add at least one trustee"})
    ...
    """
        ...
        voter_types = [
            r["user__user_type"]
            for r in self.voter_set.values("user__user_type").distinct()
            if r["user__user_type"] != None
        ]
        ...
        self.eligibility = [
            {"auth_system": auth_system} for auth_system in auth_systems
        ]
        self.save()
    ...
    """
        ...
        trustee.public_key_hash = datatypes.LDObject.instantiate(
            trustee.public_key, datatype="legacy/EGPublicKey"
        ).hash
    ...
    """
        return [
            self.one_question_winner(
                self.questions[i], self.result[i], self.num_cast_votes
            )
  ...
  """
    ...
    def process(self):
        ...
        if election.use_voter_aliases:
            voter_alias_integers = list(
                range(last_alias_num + 1, last_alias_num + 1 + num_voters)
            )
...
class Voter(HeliosModel):
    ...
    @classmethod
    def get_by_election(
        cls, election, cast=None, order_by="voter_login_id", after=None, limit=None
    ):
        """
    ...
    def generate_password(self, length=10):
        ...
        self.voter_password = heliosutils.random_string(
            length, alphabet="abcdefghjkmnopqrstuvwxyzABCDEFGHJKLMNPQRSTUVWXYZ23456789"
        )
    ...
    def last_cast_vote(self):
        return CastVote(
            vote=self.vote, vote_hash=self.vote_hash, cast_at=self.cast_at, voter=self
        )
...
class CastVote(HeliosModel):
    ...
    """
        safe_hash = self.vote_hash.decode()
        for c in ["/", "+"]:
...
class Trustee(HeliosModel):
    ...
    decryption_factors = LDObjectField(
        type_hint=datatypes.arrayOf(datatypes.arrayOf("core/BigInteger")), null=True
    )

    decryption_proofs = LDObjectField(
        type_hint=datatypes.arrayOf(datatypes.arrayOf("legacy/EGZKProof")), null=True
    )
