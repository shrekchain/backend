...
class RandomPool:
    ...
    def __init__(self, numbytes=160, cipher=None, hash=None):
        ...
        if isinstance(hash, bytes):
            ...
            hash = __import__("Crypto.Hash." + hash, None, None, ["new"])
    ...
    def stir(self, s=b""):
        ...
        for i in range(self.bytes // self._hash().digest_size):
            ...
            h.update(bytes(self.__counter) + bytes(i) + bytes(self._addPos) + s)
    ...
    def add_event(self, s=b""):
        ...
        delta = self._noise()
        s = s + long_to_bytes(event) + bytes(4 * [0xAA]) + long_to_bytes(delta)
        self._addBytes(s)
