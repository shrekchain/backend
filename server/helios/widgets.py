...
from django.forms.widgets import Select, MultiWidget, Widget, SelectDateWidget
...
class SelectTimeWidget(Widget):
    ...
    def render(self, name, value, attrs=None, renderer=None):
        ...
        if self.twelve_hr and self.meridiem_val:
            if (
                self.meridiem_val.lower().startswith("p")
                and hour_val > 12
                and hour_val < 24
            ):
...
class SplitSelectDateTimeWidget(MultiWidget):
    ...
    def __init__(
        self, attrs=None, hour_step=None, minute_step=None, twelve_hr=None, years=None
    ):
        """ pass all these parameters to their respective widget constructors..."""
        widgets = (
            SelectDateWidget(attrs=attrs, years=years),
            SelectTimeWidget(
                attrs=attrs,
                hour_step=hour_step,
    ...
    def render(self, name, value, attrs=None, renderer=None):
        rendered_widgets = list(
            widget.render(name, value, attrs=attrs, renderer=renderer)
